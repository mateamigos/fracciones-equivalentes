...
<head>
    ...
    <title>Pistolero de Fracciones - Edición Oeste</title>
    <style>
        :root { --brown: #5d4037; }
        ...
        .tomate-svg {
            position: absolute; width: 75%; height: auto; z-index: 2; opacity: 0.9; top: 22%;
        }
        ...
        .lata span {
            font-weight: 900; color: #3e2723; text-shadow: 1px 1px 0px rgba(255,255,255,0.5); font-family: 'Courier New', monospace; line-height: 1;
        }
        
        .lata hr {
            width: 80% !important; min-width: 30px; border: none !important; border-top: 3px solid #3e2723 !important; margin: 2px 0; z-index: 3; box-shadow: 0 1px 0 rgba(255,255,255,0.5);
        }
...
<body onload="cargarRecords()">
    ...
    <div id="inicio">
        <h1>PISTOLERO DE<br>FRACCIONES</h1>
        <div class="subtitulo">DISPARA A LAS FRACCIONES EQUIVALENTES</div>
        ...
        <div class="selector-container">
            <label class="selector-label" for="tipo-fraccion">TIPO DE JUEGO:</label>
            <select id="tipo-fraccion">
                <option value="ampliar">POR AMPLIACIÓN (FÁCIL)</option>
                <option value="reducir">POR REDUCCIÓN (DIFÍCIL)</option>
                <option value="ambas">AMBAS (MIXTO)</option>
            </select>
        </div>

        <div class="selector-container">
            <label class="selector-label" for="max-valor">MÁXIMO NÚMERO (10-100):</label>
            <input type="number" id="max-valor" min="10" max="100" value="20">
        </div>
    ...
    <div id="juego">
        <div id="pizarra">
            ...
            <div>WANTED: <span id="obj-texto" style="color:#ffcc00">1/2</span></div>
    ...
    <script>
        ...
        const svgTomate = `
        <svg class="tomate-svg" viewBox="0 0 100 100">
            <ellipse cx="50" cy="85" rx="30" ry="5" fill="rgba(0,0,0,0.1)" />
            <circle cx="50" cy="55" r="35" fill="#d32f2f" stroke="#b71c1c" stroke-width="2"/>
            <path d="M50 25 L55 40 L65 35 L60 45 L70 50 L55 50 L50 65 L45 50 L30 50 L40 45 L35 35 L45 40 Z" fill="#388e3c" stroke="#2e7d32" stroke-width="1" />
        </svg>
        `;
        ...
        // --- GENERACIÓN DE NIVEL MEJORADA (DISTRACTORES INTELIGENTES) ---
        function generarNivel() {
            ...
            let maxValInput = parseInt(document.getElementById('max-valor').value);
            if (isNaN(maxValInput) || maxValInput < 10) maxValInput = 10;
            if (maxValInput > 100) maxValInput = 100;
            const maxValor = maxValInput;

            const allBaseFractions = [
                {n:1, d:2}, {n:1, d:3}, {n:2, d:3}, {n:1, d:4}, {n:3, d:4}, 
                {n:1, d:5}, {n:2, d:5}, {n:3, d:5}, {n:4, d:5}, 
                {n:1, d:6}, {n:5, d:6}, {n:1, d:7}, {n:2, d:7}, {n:3, d:7},
                {n:1, d:8}, {n:3, d:8}, {n:5, d:8}, {n:7, d:8}, {n:1, d:9}
            ];

            let validBases = allBaseFractions.filter(f => (f.d * 2) <= maxValor);
            if (validBases.length === 0) validBases = [{n:1, d:2}];
            let base = validBases[Math.floor(Math.random() * validBases.length)];
            let maxFactorPossible = Math.floor(maxValor / base.d);
            let factor = Math.floor(Math.random() * (maxFactorPossible - 1)) + 2; 
            let fraccionAmpliada = { n: base.n * factor, d: base.d * factor };
            
            let tipoSelector = document.getElementById('tipo-fraccion').value;
            let esReduccion = (tipoSelector === 'reducir') ? true : (tipoSelector === 'ambas' ? Math.random() < 0.5 : false);

            let fraccionObjetivoVisual = esReduccion ? fraccionAmpliada : base;
            let fraccionRespuestaCorrecta = esReduccion ? base : fraccionAmpliada;

            document.getElementById('obj-texto').innerText = `${fraccionObjetivoVisual.n}/${fraccionObjetivoVisual.d}`;

            let opciones = [];
            opciones.push({ n: fraccionRespuestaCorrecta.n, d: fraccionRespuestaCorrecta.d, correcta: true });

            // LOGICA MEJORADA DE DISTRACTORES
            while(opciones.length < 4) {
                let n_dist, d_dist;
                const seed = Math.random();
                
                // Estrategia 1: Mismo denominador que la correcta, numerador diferente (confusión alta)
                if (seed < 0.33) {
                    d_dist = fraccionRespuestaCorrecta.d;
                    let offset = (Math.random() < 0.5 ? -1 : 1) * (Math.floor(Math.random() * 2) + 1);
                    n_dist = fraccionRespuestaCorrecta.n + offset;
                } 
                // Estrategia 2: Mismo numerador que la correcta, denominador diferente
                else if (seed < 0.66) {
                    n_dist = fraccionRespuestaCorrecta.n;
                    let offset = (Math.random() < 0.5 ? -1 : 1) * (Math.floor(Math.random() * 2) + 1);
                    d_dist = fraccionRespuestaCorrecta.d + offset;
                }
                // Estrategia 3: "Falsa familia". Multiplos de la base pero incoherentes.
                else {
                     let mN = Math.floor(Math.random() * 5) + 2;
                     let mD = Math.floor(Math.random() * 5) + 2;
                     // Evitar que sean iguales para no crear una equivalente por accidente
                     if (mN === mD) mD++; 
                     n_dist = base.n * mN;
                     d_dist = base.d * mD;
                }
                ...
                if (n_dist <= 0) n_dist = 1;
                if (d_dist <= 0) d_dist = 2;
                // Intentar mantener la proporción propia/impropia similar si es posible, aunque la prioridad es el distractor numérico
                if (fraccionRespuestaCorrecta.n < fraccionRespuestaCorrecta.d && n_dist >= d_dist) {
                    // Si la respuesta es propia y el distractor salió impropio, invierte
                    let temp = n_dist; n_dist = d_dist; d_dist = temp;
                }

                // Verificar que NO sea equivalente a la correcta (n1*d2 == n2*d1)
                let esEquivalente = (n_dist * fraccionRespuestaCorrecta.d === d_dist * fraccionRespuestaCorrecta.n);
                
                // Verificar que no esté ya en la lista
                let existe = opciones.some(o => o.n === n_dist && o.d === d_dist);

                if (!esEquivalente && !existe) {
                    opciones.push({ n: n_dist, d: d_dist, correcta: false });
                }
            }
            ...
            opciones.forEach((opt, index) => {
                ...
                div.className = 'lata';
                div.innerHTML = `${svgTomate}<div class="contenido-fraccion"><span>${opt.n}</span><hr><span>${opt.d}</span></div>`;
                div.onclick = (e) => {
                    e.stopPropagation(); 
